...
class IngredientsAnalyzer(CacheableMixin):
    ...
    def normalize_ingredient(self, txt: str) -> str:
        ...
        # Liste optimisée de mots non importants (50 mots essentiels)
        stop_words = {
            # Taille/quantité (10 mots)
            "large", "small", "medium", "extra", "whole",
            "sliced", "diced", "chopped", "minced", "ground",
            # Couleurs (6 mots)
            "black", "white", "red", "green", "dark", "light",
            # États/préparations (12 mots)
    ...
    def clean_ingredient(self, ingredient: str) -> str:
        ...
        ingredient = re.sub(
            r"[0-9]+\.?[0-9]*\s*(cups?|tablespoons?|teaspoons?|ounces?|pounds?|grams?|ml|l)",
            "",
            ingredient,
        )
    ...
    def get_processing_summary(self) -> dict:
        ...
        return {
            ...
            "cooccurrence_matrix": {
                ...
                "sparsity": round(
                    (1 - non_zero_pairs / (matrix_size * (matrix_size - 1) / 2)) * 100,
                    1,
                ),
            },
